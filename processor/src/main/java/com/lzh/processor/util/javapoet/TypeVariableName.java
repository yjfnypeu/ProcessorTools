...
public final class TypeVariableName extends com.lzh.processor.util.javapoet.TypeName {
  public final String name;
  public final List<com.lzh.processor.util.javapoet.TypeName> bounds;

  private TypeVariableName(String name, List<com.lzh.processor.util.javapoet.TypeName> bounds) {
    this(name, bounds, new ArrayList<com.lzh.processor.util.javapoet.AnnotationSpec>());
  }

  private TypeVariableName(String name, List<com.lzh.processor.util.javapoet.TypeName> bounds, List<com.lzh.processor.util.javapoet.AnnotationSpec> annotations) {
    super(annotations);
    this.name = checkNotNull(name, "name == null");
    this.bounds = bounds;

    for (com.lzh.processor.util.javapoet.TypeName bound : this.bounds) {
      checkArgument(!bound.isPrimitive() && bound != VOID, "invalid bound: %s", bound);
    }
  }

  @Override public TypeVariableName annotated(com.lzh.processor.util.javapoet.AnnotationSpec... annotations) {
    return annotated(Arrays.asList(annotations));
  }

  @Override public TypeVariableName annotated(List<com.lzh.processor.util.javapoet.AnnotationSpec> annotations) {
    return new TypeVariableName(name, bounds, annotations);
  }

  private static TypeVariableName of(String name, List<com.lzh.processor.util.javapoet.TypeName> bounds) {
    // Strip java.lang.Object from bounds if it is present.
    List<com.lzh.processor.util.javapoet.TypeName> boundsNoObject = new ArrayList<>(bounds);
  ...
  public static TypeVariableName get(String name) {
    return TypeVariableName.of(name, Collections.<com.lzh.processor.util.javapoet.TypeName>emptyList());
  }

  /** Returns type variable named {@code name} with {@code bounds}. */
  public static TypeVariableName get(String name, com.lzh.processor.util.javapoet.TypeName... bounds) {
    return TypeVariableName.of(name, Arrays.asList(bounds));
  }

  /** Returns type variable named {@code name} with {@code bounds}. */
  public static TypeVariableName get(String name, Type... bounds) {
    return TypeVariableName.of(name, com.lzh.processor.util.javapoet.TypeName.list(bounds));
  }
  ...
  static TypeVariableName get(
    ...
    if (typeVariableName == null) {
      ...
      List<com.lzh.processor.util.javapoet.TypeName> bounds = new ArrayList<>();
      List<com.lzh.processor.util.javapoet.TypeName> visibleBounds = Collections.unmodifiableList(bounds);
      typeVariableName = new TypeVariableName(element.getSimpleName().toString(), visibleBounds);
      typeVariables.put(element, typeVariableName);
      for (TypeMirror typeMirror : element.getBounds()) {
        bounds.add(com.lzh.processor.util.javapoet.TypeName.get(typeMirror, typeVariables));
      }
  ...
  public static TypeVariableName get(TypeParameterElement element) {
    ...
    List<com.lzh.processor.util.javapoet.TypeName> boundsTypeNames = new ArrayList<>();
    for (TypeMirror typeMirror : boundsMirrors) {
      boundsTypeNames.add(com.lzh.processor.util.javapoet.TypeName.get(typeMirror));
    }
